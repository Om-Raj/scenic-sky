...
// Moved original page logic here to satisfy Suspense requirement for useSearchParams
...
// Extend maplibre-gl Map instance with our scenic popup helpers (typed to avoid any)
interface MapWithScenic extends maplibregl.Map {
  showScenicPopup?: (location: ScenicLocationWithDetection) => void;
  closeScenicPopup?: () => void;
}

export default function FlightMapPageContent() {
  ...
  const handleClosePopup = useCallback(() => {
    ...
    if (map) {
      const m = map as unknown as MapWithScenic;
      m.closeScenicPopup?.();
    }
  ...
  useEffect(() => {
    ...
    if (locationsToTrigger.length > 0) {
      ...
      const m = map as unknown as MapWithScenic;
      m.showScenicPopup?.(locationToShow);
  ...
  // Reset handled via onReset passed to FlightPath; we keep helpers local when needed
  ...
  // Reset animation and clear scenic state/popup
